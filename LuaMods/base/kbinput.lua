...
local key_to_mouse = { left_button = left_btn, right_button = right_btn, middle_button = middle_btn, 
	x_button_1 = xmouse_1, x_button_2 = xmouse_2, x_button_3 = xmouse_3, x_button_4 = xmouse_4, 
	x_button_5 = xmouse_5, wheel_up = wheel_up, wheel_down = wheel_down  
}
...
-- Function, that returns void if require haven't returned callback function
local function handled_callback( path )
	local clbk = require(path)
...
function keyinput:update(t, dt)
	for key,value in pairs(self.keys) do
		if pressed(key, value.no_stuck) then--and (value.ig_chat) then
